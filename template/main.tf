...
data "coder_parameter" "workspace_mode" {
  ...
  mutable = false
  order = 2
  option { name = "Development"
  value = "development" }
  option { name = "CI repair"
  value = "ci-repair" }
  option { name = "Governance"
  value = "governance" }
  option { name = "Investigation"
  value = "investigation" }
  option { name = "Validator"
  value = "validator" }
  option { name = "Staging validation"
  value = "staging-validation" }
  option { name = "Release validation"
...
data "coder_parameter" "inference_provider" {
  ...
  option { name = "None"
  value = "none" }
  option { name = "OpenAI-compatible vLLM"
  value = "vllm" }
}
data "coder_parameter" "vllm_base_url" { name = "vllm_base_url"
  display_name = "vLLM base URL"
  default = ""
  mutable = true
  order = 4 }
data "coder_parameter" "vllm_model" { name = "vllm_model"
  display_name = "vLLM model"
  default = ""
  mutable = true
  order = 5 }
data "coder_parameter" "vllm_context_window" { name = "vllm_context_window"
  display_name = "vLLM context window"
  type = "number"
  default = "32768"
  mutable = true
  order = 6 }
data "coder_parameter" "copilot_offline" { name = "copilot_offline"
  display_name = "Local model traffic only"
  type = "bool"
  default = false
  mutable = true
  order = 7 }
data "coder_parameter" "container_validation_enabled" { name = "container_validation_enabled"
...
resource "coder_agent" "main" {
  ...
  metadata { display_name = "Mode"
  key = "mode"
  script = "echo ${data.coder_parameter.workspace_mode.value}"
  interval = 0 }
  metadata { display_name = "Repository HEAD"
  key = "head"
  script = "git -C /workspaces/Bibliophilarr rev-parse --short HEAD 2>/dev/null || echo pending"
  interval = 60 }
  metadata { display_name = "Docker"
  key = "docker"
  script = "docker info >/dev/null 2>&1 && echo available || echo unavailable"
  interval = 60 }
}

resource "docker_volume" "home" { name = "coder-${data.coder_workspace_owner.me.name}-${data.coder_workspace.me.name}-home"
  lifecycle { ignore_changes = all } }
resource "docker_volume" "nuget" { name = "coder-${data.coder_workspace_owner.me.name}-${data.coder_workspace.me.name}-nuget"
  lifecycle { ignore_changes = all } }
resource "docker_volume" "yarn" { name = "coder-${data.coder_workspace_owner.me.name}-${data.coder_workspace.me.name}-yarn"
  lifecycle { ignore_changes = all } }
...
resource "docker_container" "workspace" {
  ...
  volumes { volume_name = docker_volume.home.name
  container_path = "/home/coder" }
  volumes { volume_name = docker_volume.nuget.name
  container_path = "/home/coder/.nuget/packages" }
  volumes { volume_name = docker_volume.yarn.name
  ...
  networks_advanced { name = docker_network.workspace.name }
}
resource "docker_network" "workspace" { name = "coder-${data.coder_workspace_owner.me.name}-${data.coder_workspace.me.name}" }
