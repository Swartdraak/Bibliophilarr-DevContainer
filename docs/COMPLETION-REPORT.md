...
| 8  | JetBrains first-launch reliability + BYOK path | **PASS_WITH_LIMITATIONS** | deterministic: pinned builds via `ide_config` (`.NET` build downloads once to the persistent backend) + Gateway handshake 0-1 s; the Copilot plugin **is installable** into the persistent plugins path, and once the user **authenticates** (one-time, not scriptable) **Copilot BYOK can target the local endpoint** — the auth login is the only first-launch step that blocks a fully headless path; AI Assistant is an alternative vehicle |
